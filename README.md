...
:warning: Make sure you have [Vue CLI 3+](https://github.com/vuejs/vue-cli) installed:
...
The `patterns` property must either be an object or an array of objects. Each pattern must contain both `match` and `replacement` options, otherwise it would throw an exception. Errors are ignored in `silent` mode, thus patterns wouldn't be applied.
...
In case you'd need to prevent patterns to be applied to some of your pages, you can specify either one of `excludes` or `includes` options. If both options are set, then `excludes` will be ignored in `silent` mode.
